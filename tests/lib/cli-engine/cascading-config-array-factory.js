...
/**
 * Creates a stubbed CascadingConfigArrayFactory object that will use the in-memory file system.
 * @param {Object} options The options for CascadingConfigArrayFactory.
 * @param {Object} files The file definitions in the in-memory file system.
 * @returns {CascadingConfigArrayFactory} The stubbed instance of Config.
 * @private
 */
function createStubbedCascadingConfigArrayFactory(options, files) {
    const { CascadingConfigArrayFactory: StubbedCascadingConfigArrayFactory } =
        defineCascadingConfigArrayFactoryWithInmemoryFileSystem({ files });

    return new StubbedCascadingConfigArrayFactory(options);
}
        ...
        describe("Moved from tests/lib/config.js", () => {
            ...
            describe("getConfig()", () => {
                ...
                it("should return a modified config without plugin rules enabled when baseConfig is set to an object with plugin and no .eslintrc", () => {
                    const factory = createStubbedCascadingConfigArrayFactory(
                        {
                            baseConfig: {
                                env: {
                                    node: true
                ...
                describe("personal config file within home directory", () => {
                    ...
                    it("should load the personal config if no local config was found", () => {
                        ...
                        const factory = createStubbedCascadingConfigArrayFactory({ cwd: projectPath }, {
